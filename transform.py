...
class Matrix(tuple):
    def __new__(cls, *nums): tuple.__new__(cls, nums[0] if len(nums) == 1 else nums)
    def __repr__(s):
    ...
    def submatrix(s, row, col):
        ...
        """
        return s.__class__(tuple(s[r][c] for c in range(4) if c != col) for r in range(4) if r != row)
    def zip(m1, m2):
        """
